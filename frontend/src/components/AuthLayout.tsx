...
export function AuthLayout({ title, subtitle, onSubmit, children, footer }: Props) {
  return (
    <div className="desk-glow min-h-screen lg:grid lg:grid-cols-[1.05fr_1fr]">
      ...
      <section className="grid min-h-screen place-items-center px-5 py-10 lg:min-h-0">
        <form
          onSubmit={onSubmit}
          className="relative w-full max-w-100 animate-sheet-in rounded-xs border border-edge bg-sheet px-8 pt-9 pb-7 shadow-2xl before:absolute before:-top-3 before:left-1/2 before:h-6 before:w-28 before:-translate-x-1/2 before:-rotate-2 before:border-x before:border-dashed before:border-ink/10 before:bg-accent/20 before:content-['']"
        >
          <NotebookArt className="mx-auto mb-6 max-w-48 lg:hidden" />
